...
def inspect_template(path: str | Path) -> TemplateInspection:
    ...
    for sec in doc.sections:
        w = float(sec.page_width.mm); h = float(sec.page_height.mm)
    ...
    return TemplateInspection(
        path=str(source), sections=len(doc.sections), page_sizes=page_sizes, margins_mm=margins,
        styles=len(styles), paragraph_styles=paragraph, character_styles=character,
        table_styles=table, custom_styles=custom, header_text=headers, footer_text=footers,
