...
def load_entrypoint_extensions(
    names: tuple[str, ...], group: str = "mddocx.extensions"
) -> tuple[Any, ...]:
    ...
    selected = (
        available.select(group=group) if hasattr(available, "select") else available.get(group, [])
    )
    ...
    for name in requested:
        ...
        if not matches:
            raise MddocxError(
                Diagnostic("error", "PLUGIN401", f"Extension entry point not found: {name}")
            )
        if len(matches) > 1:
            raise MddocxError(
                Diagnostic(
                    "error", "PLUGIN402", f"Multiple extension entry points share the name: {name}"
                )
            )
        try:
            ...
            raise MddocxError(
                Diagnostic("error", "PLUGIN403", f"Unable to load extension entry point: {name}")
            ) from exc
    return tuple(loaded)
