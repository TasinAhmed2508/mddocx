...
class DocxRenderer:
    ...
    def _open_document(self):
        ...
        try:
            validate_docx_package(template.read_bytes(), self.config.validation)
        ...
        def render_header(header, text: str | None, include_defaults: bool = True) -> None:
            ...
            if pieces:
                render_field_template(
                    p,
                    " — ".join(pieces),
                    {"TITLE": self.config.title or "", "AUTHOR": self.config.author or ""},
                )
    ...
    def _render_block(self, node, list_ctx=None, list_level=0):
        ...
        if isinstance(node, Heading):
            ...
            if self.config.heading_bookmarks:
                ...
                target = (
                    self.references.get(node.identifier)
                    if node.identifier and self.references
                    else None
                )
                ...
                self._bookmark_id += 1
            if (
                self.config.references.enabled
                and node.level == 1
                and (
            ...
            if (node.language or "").strip().lower() == "mermaid" and self.config.mermaid.enabled:
                ...
                try:
                    ...
                    diagram_caption = (
                        ...
                        if self.config.references.captions
                        else None
                    )
                    if (
                        diagram_caption
                    ...
                    self._add_image_path(
                        p, path, diagram_caption or "Mermaid diagram", title=diagram_caption
                    )
                    if (
                        diagram_caption
                        and self.config.references.figure_caption_position == "below"
                    ):
                        self._render_caption("Figure", diagram_caption, node.identifier)
                    self.reporter.info(
                        "DIAGRAM101",
            ...
            if (
                self.config.references.enabled
                and node.identifier
                and self.config.references.equation_numbering
            ):
            ...
            caption = (
                (
                    node.caption
                    or (node.title if self.config.image_captions_from_title else None)
            ...
            self._configure_section(
                sec,
                preserve_page=bool(
                    self.config.template and self.config.preserve_template_page_setup
                ),
            )
        else:
    ...
    def _prepare_chart_entry(self, node: ChartBlock) -> ChartEntry:
        ...
        if node.source_path:
            ...
            data = load_tabular_data(
                path, max_rows=self.config.data.max_rows, max_columns=self.config.data.max_columns
            )
            if not data.columns:
                raise MddocxError(
                    Diagnostic(
        ...
        if len(raw_series) > self.config.charts.max_series:
            raise MddocxError(
                Diagnostic(
                    "error",
                    "CHART204",
                    f"Chart exceeds {self.config.charts.max_series} series.",
                    getattr(node.source, "file", None),
                    getattr(node.source, "line", None),
                )
            )
        ...
        if max_points > self.config.charts.max_points:
            raise MddocxError(
                Diagnostic(
                    "error",
                    "CHART206",
                    f"Chart exceeds {self.config.charts.max_points} data points.",
                    getattr(node.source, "file", None),
                    getattr(node.source, "line", None),
                )
            )
    ...
    def _render_data_table(self, node: DataTableBlock) -> None:
        ...
        for raw in data.rows:
            cells = [
                TableCell(
                    children=[
                        Text(text="" if idx >= len(raw) or raw[idx] is None else str(raw[idx]))
                    ]
                )
                for idx in indices
            ]
    ...
    def _render_caption(self, label: str, caption: str, identifier: str | None) -> None:
        ...
        if target and target.number is not None:
            if (
                self.config.references.caption_number_format == "section"
                and isinstance(target.number, str)
                and "." in target.number
            ):
                section_no, item_no = target.number.split(".", 1)
                add_chapter_seq_field(
                    p, label, section_no, item_no, target.bookmark, self._bookmark_id
                )
            else:
    ...
    def _render_numbered_equation(self, node: MathBlock) -> None:
        ...
        if target and target.number is not None:
            if (
                ...
                add_chapter_seq_field(
                    np, "Equation", section_no, equation_no, target.bookmark, self._bookmark_id
                )
            else:
                add_seq_field(
                    np, "Equation", str(target.number), target.bookmark, self._bookmark_id
                )
            self._bookmark_id += 1
        else:
    ...
    def _render_cross_reference(self, paragraph, node: CrossReference) -> None:
        ...
        if not self.config.references.enabled:
            if (
                prefix is None
                and self.config.references.include_prefix_in_crossrefs
    ...
    def _render_table_with_layout(self, node: Table) -> None:
        ...
        if (
            ...
            and self.config.table.restore_portrait_after_landscape
        ):
            sec = self.document.add_section(WD_SECTION.NEW_PAGE)
            self._configure_section(
                sec,
    ...
    def _table_should_landscape(self, node: Table) -> bool:
        decision = self._layout_plan.for_table(node)
        if decision is not None:
            return (
                decision.orientation == "landscape" and self._current_orientation() != "landscape"
            )
        if (
            not self.config.table.auto_landscape
            or self._current_orientation() == "landscape"
            or not node.rows
        ):
            return False
    ...
    def _render_code_block(self, node: CodeBlock) -> None:
        ...
        show_label = (
            self.config.code.show_language_label
        ...
        for line_no, raw_line in enumerate(lines, 1):
            ...
            if not tokens:
                ...
                self._configure_run(run, raw_line, code=True)
                continue
            for token_type, token_text in tokens:
                ...
                self._configure_run(run, token_text, code=True)
    ...
    def _add_image_path(
        ...
        requested_percent = (
            ...
            else self.config.figures.default_width_percent
        )
        ...
        if shape.width > max_width or (
            self.config.images.allow_upscale and shape.width < max_width
        ):
