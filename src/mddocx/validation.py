    ...
    try:
        with ZipFile(BytesIO(blob), "r") as archive:
            ...
            if len(names) > config.max_package_parts:
                _fail(
                    "DOCX402", f"DOCX contains more than {config.max_package_parts} package parts."
                )
            ...
            if total_uncompressed > config.max_total_uncompressed_size:
                _fail(
                    "DOCX407",
                    f"DOCX uncompressed package size exceeds {config.max_total_uncompressed_size} bytes.",
                )
            if not config.allow_macros and any(
        ...
        if inspection.duplicate_parts:
            _fail(
                "DOCX409",
                "DOCX contains duplicate package part names: "
                + ", ".join(inspection.duplicate_parts[:5]),
            )
        if config.validate_relationships and inspection.broken_relationships:
            _fail(
                "DOCX410",
                "DOCX contains broken internal relationships: "
                + "; ".join(inspection.broken_relationships[:3]),
            )
        if config.validate_math_structure and inspection.empty_nary_operands:
            _fail(
                "DOCX411",
                f"DOCX contains {inspection.empty_nary_operands} n-ary math operators with empty operands.",
            )
        if config.validate_math_structure and inspection.radicals_missing_degree:
            _fail(
                "DOCX412",
                f"DOCX contains {inspection.radicals_missing_degree} malformed radical structures.",
            )
        ...
        if config.validate_relationships and inspection.dangling_internal_hyperlinks:
            _fail(
                "DOCX414",
                f"DOCX contains {inspection.dangling_internal_hyperlinks} dangling internal hyperlinks.",
            )
        if config.validate_relationships and inspection.dangling_ref_fields:
            _fail(
                "DOCX415",
                f"DOCX contains {inspection.dangling_ref_fields} REF fields targeting missing bookmarks.",
            )
        if inspection.duplicate_bookmark_names:
            _fail(
                "DOCX416",
                f"DOCX contains {inspection.duplicate_bookmark_names} duplicate bookmark names.",
            )
        ...
        if inspection.duplicate_relationship_ids:
            _fail(
                "DOCX418",
                f"DOCX contains {inspection.duplicate_relationship_ids} duplicate relationship IDs.",
            )
        if inspection.duplicate_drawing_ids:
            _fail(
                "DOCX419",
                f"DOCX contains {inspection.duplicate_drawing_ids} duplicate drawing object IDs.",
            )
        if inspection.duplicate_note_ids:
            _fail(
                "DOCX420",
                f"DOCX contains {inspection.duplicate_note_ids} duplicate footnote/endnote IDs.",
            )
        if inspection.chart_relationship_issues:
            _fail(
                "DOCX421",
                f"DOCX contains {inspection.chart_relationship_issues} invalid chart/workbook relationship sets.",
            )
    except MddocxError:
        raise
    except (BadZipFile, OSError, ValueError) as exc:
        raise MddocxError(
            Diagnostic("error", "DOCX401", "Generated output is not a valid DOCX ZIP package.")
        ) from exc
