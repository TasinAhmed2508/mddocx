...
def inject_charts(blob: bytes, entries: list[ChartEntry]) -> bytes:
    ...
    for index, entry in enumerate(entries, start=1):
        ...
        rel.set("Id", rid); rel.set("Type", CHART_REL); rel.set("Target", f"charts/{chart_name}")
        paragraph = _find_placeholder_paragraph(document, entry.token)
        if paragraph is None:
            raise MddocxError(Diagnostic("error", "CHART301", "Internal chart placeholder was not found."))
        ...
        _ensure_override(content_types, f"/word/charts/{chart_name}", "application/vnd.openxmlformats-officedocument.drawingml.chart+xml")
    _ensure_default(content_types, "xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    parts["word/document.xml"] = etree.tostring(document, xml_declaration=True, encoding="UTF-8", standalone="yes")
    parts["word/_rels/document.xml.rels"] = etree.tostring(rels, xml_declaration=True, encoding="UTF-8", standalone="yes")
    parts["[Content_Types].xml"] = etree.tostring(content_types, xml_declaration=True, encoding="UTF-8", standalone="yes")
...
def _replace_with_drawing(paragraph, rid: str, entry: ChartEntry, docpr_id: int) -> None:
    ...
    cx = str(int(entry.width_mm * 36000)); cy = str(int(entry.height_mm * 36000))
    extent = etree.SubElement(inline, f"{{{WP}}}extent"); extent.set("cx", cx); extent.set("cy", cy)
    effect = etree.SubElement(inline, f"{{{WP}}}effectExtent")
    for key in ("l", "t", "r", "b"): effect.set(key, "0")
    docpr = etree.SubElement(inline, f"{{{WP}}}docPr")
    docpr.set("id", str(docpr_id)); docpr.set("name", f"Chart {docpr_id}"); docpr.set("descr", entry.alt_text or entry.title or "Chart")
    cnv = etree.SubElement(inline, f"{{{WP}}}cNvGraphicFramePr")
    locks = etree.SubElement(cnv, f"{{{A}}}graphicFrameLocks"); locks.set("noChangeAspect", "1")
    graphic = etree.SubElement(inline, f"{{{A}}}graphic")
    data = etree.SubElement(graphic, f"{{{A}}}graphicData"); data.set("uri", C)
    chart = etree.SubElement(data, f"{{{C}}}chart"); chart.set(f"{{{R}}}id", rid)
        ...
        for name in sorted(zin.namelist()):
            ...
            if name == "docProps/core.xml":
                try:
                    ...
                    for node in root.xpath("//*[local-name()='created' or local-name()='modified']"):
...
def _chart_xml(entry: ChartEntry, workbook_name: str) -> bytes:
    ...
    secondary_indexes = [i for i, (name, _values) in enumerate(entry.series) if name in secondary_names]
    ...
    if kind in {"bar", "column"}:
        ...
        raise MddocxError(Diagnostic("error", "CHART202", f"Unsupported chart type: {entry.chart_type}"))
...
def _legend_code(value: str) -> str | None:
    return {
        ...
        "top": "t", "t": "t", "bottom": "b", "b": "b",
        "top_right": "tr", "tr": "tr", "none": None, "off": None,
...
def _category_series(parent, entry: ChartEntry, line: bool = False, indexes: list[int] | None = None) -> None:
    ...
    for idx in indexes:
        ...
        _str_ref(etree.SubElement(ser, f"{{{C}}}cat"), "Sheet1!$A$2:$A$%d" % (rows + 1), entry.categories)
...
def _series_name(ser, column: int, name: str) -> None:
    tx = etree.SubElement(ser, f"{{{C}}}tx"); ref = etree.SubElement(tx, f"{{{C}}}strRef")
    etree.SubElement(ref, f"{{{C}}}f").text = f"Sheet1!${_col(column)}$1"
    cache = etree.SubElement(ref, f"{{{C}}}strCache"); etree.SubElement(cache, f"{{{C}}}ptCount").set("val", "1")
    pt = etree.SubElement(cache, f"{{{C}}}pt"); pt.set("idx", "0"); etree.SubElement(pt, f"{{{C}}}v").text = name


def _str_ref(parent, formula: str, values: list[object]) -> None:
    ref = etree.SubElement(parent, f"{{{C}}}strRef"); etree.SubElement(ref, f"{{{C}}}f").text = formula
    cache = etree.SubElement(ref, f"{{{C}}}strCache"); etree.SubElement(cache, f"{{{C}}}ptCount").set("val", str(len(values)))
    for idx, value in enumerate(values):
        pt = etree.SubElement(cache, f"{{{C}}}pt"); pt.set("idx", str(idx)); etree.SubElement(pt, f"{{{C}}}v").text = "" if value is None else str(value)


def _num_ref(parent, formula: str, values: list[object]) -> None:
    ref = etree.SubElement(parent, f"{{{C}}}numRef"); etree.SubElement(ref, f"{{{C}}}f").text = formula
    cache = etree.SubElement(ref, f"{{{C}}}numCache"); etree.SubElement(cache, f"{{{C}}}formatCode").text = "General"
    etree.SubElement(cache, f"{{{C}}}ptCount").set("val", str(len(values)))
    for idx, value in enumerate(values):
        pt = etree.SubElement(cache, f"{{{C}}}pt"); pt.set("idx", str(idx))
        try: text = str(float(value))
        except (TypeError, ValueError): text = "0"
        etree.SubElement(pt, f"{{{C}}}v").text = text


def _axes(plot, chart_node, entry: ChartEntry) -> None:
    _category_axis_pair(
        plot, chart_node, entry,
        cat_id="123456", val_id="123457", cat_pos="b", val_pos="l",
        delete_cat=False, secondary=False,
    )
...
def _secondary_axes(plot, chart_node, entry: ChartEntry) -> None:
    _category_axis_pair(
        plot, chart_node, entry,
        cat_id="123458", val_id="123459", cat_pos="t", val_pos="r",
        delete_cat=True, secondary=True,
    )


def _category_axis_pair(plot, chart_node, entry: ChartEntry, *, cat_id: str, val_id: str, cat_pos: str, val_pos: str, delete_cat: bool, secondary: bool) -> None:
    ...
    etree.SubElement(val, f"{{{C}}}numFmt", formatCode=number_format or "General", sourceLinked="0" if number_format else "1")
...
def _value_axes(plot, chart_node, entry: ChartEntry) -> None:
    ...
    etree.SubElement(x, f"{{{C}}}numFmt", formatCode=entry.x_number_format or "General", sourceLinked="0" if entry.x_number_format else "1")
    ...
    etree.SubElement(y, f"{{{C}}}numFmt", formatCode=entry.y_number_format or "General", sourceLinked="0" if entry.y_number_format else "1")
...
def _formula(column: int, rows: int) -> str:
    col = _col(column); return f"Sheet1!${col}$2:${col}${rows + 1}"
...
def _chart_rels(workbook_name: str) -> bytes:
    ...
    rel.set("Id", "rId1"); rel.set("Type", PACKAGE_REL); rel.set("Target", f"../embeddings/{workbook_name}")
    return etree.tostring(root, xml_declaration=True, encoding="UTF-8", standalone="yes")
...
def _next_rid(rels) -> int:
    ...
    for rel in rels:
        rid = rel.get("Id", "")
        if rid.startswith("rId") and rid[3:].isdigit(): values.append(int(rid[3:]))
...
def _next_docpr(document) -> int:
    ...
    for node in document.xpath(".//*[local-name()='docPr']"):
        try: ids.append(int(node.get("id", "0")))
        except ValueError: pass
    return max(ids, default=0) + 1


def _ensure_override(root, part_name: str, content_type: str) -> None:
    if root.xpath(f"./ct:Override[@PartName='{part_name}']", namespaces={"ct": CT}): return
    node = etree.SubElement(root, f"{{{CT}}}Override"); node.set("PartName", part_name); node.set("ContentType", content_type)


def _ensure_default(root, extension: str, content_type: str) -> None:
    if root.xpath(f"./ct:Default[@Extension='{extension}']", namespaces={"ct": CT}): return
    node = etree.SubElement(root, f"{{{CT}}}Default"); node.set("Extension", extension); node.set("ContentType", content_type)
