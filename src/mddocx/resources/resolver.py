...
class _SafeRedirectHandler(HTTPRedirectHandler):
    ...
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        ...
        if self.count > self.resolver.policy.max_redirects:
            raise MddocxError(Diagnostic("error", "RESOURCE210", "Remote resource exceeded redirect limit."))
...
class ResourceResolver:
    def __init__(self, base_dir: Path, policy: ResourcePolicy, images: ImageConfig | None = None):
        ...
        self.cache_dir = Path(policy.cache_directory).expanduser().resolve() if policy.cache_directory else None
    ...
    def _resolve_local(self, source: str) -> Path:
        ...
        try:
            ...
            raise MddocxError(Diagnostic("error", "RESOURCE203", "Resource path escapes the Markdown directory.")) from exc
        ...
        if candidate.stat().st_size > self.policy.max_resource_size:
            raise MddocxError(Diagnostic("error", "RESOURCE205", "Resource exceeds configured size limit."))
        ...
        try:
            with opener.open(req, timeout=self.policy.timeout_seconds) as response:
                ...
                if self.policy.validate_mime and content_type not in _IMAGE_MIMES:
                    raise MddocxError(Diagnostic("error", "RESOURCE206", f"Unsupported remote MIME type: {content_type}"))
                suffix = _IMAGE_MIMES.get(content_type) or Path(urlparse(final_url).path).suffix.lower() or ".img"
                target_dir = self.cache_dir if self.cache_dir and self.policy.cache_remote_resources else self.temp_dir
                ...
                with target.open("wb") as f:
                    while True:
                        ...
                        if total > self.policy.max_resource_size:
                            ...
                            raise MddocxError(Diagnostic("error", "RESOURCE205", "Remote resource exceeds configured size limit."))
                        f.write(chunk)
        except MddocxError:
            raise
        except (HTTPError, URLError, TimeoutError, OSError) as exc:
            raise MddocxError(Diagnostic("error", "RESOURCE207", f"Unable to download remote resource: {source}")) from exc
    ...
    def _validate_remote_url(self, url: str) -> None:
        ...
        if scheme not in {s.lower() for s in self.policy.allowed_schemes}:
            raise MddocxError(Diagnostic("error", "RESOURCE208", f"Remote URL scheme is not allowed: {scheme or '(none)'}"))
        ...
        if self.policy.allowed_domains:
            ...
            if not any(host == d or host.endswith("." + d) for d in allowed):
                raise MddocxError(Diagnostic("error", "RESOURCE209", f"Remote host is not in the allowed domain list: {host}"))
        if not self.policy.allow_private_hosts:
            try:
                infos = socket.getaddrinfo(host, parsed.port or 443, type=socket.SOCK_STREAM)
            except socket.gaierror as exc:
                raise MddocxError(Diagnostic("error", "RESOURCE207", f"Unable to resolve remote host: {host}")) from exc
            for info in infos:
                address = ipaddress.ip_address(info[4][0])
                if not address.is_global:
                    raise MddocxError(Diagnostic("error", "RESOURCE211", "Private, loopback, link-local, or non-global remote hosts are blocked."))
    ...
    def _prepare_image(self, path: Path, source_hint: str) -> Path:
        ...
        raise MddocxError(Diagnostic("error", "IMAGE201", f"Unsupported image type: {guessed or suffix or 'unknown'}"))
    ...
    def _verify_raster(path: Path) -> None:
        try:
            ...
            raise MddocxError(Diagnostic("error", "IMAGE204", f"Invalid raster image: {path.name}")) from exc

    def _convert_webp(self, path: Path) -> Path:
        try:
            from PIL import Image
        except ImportError as exc:
            raise MddocxError(Diagnostic("error", "IMAGE205", "WebP conversion requires Pillow.")) from exc
        ...
        try:
            ...
            raise MddocxError(Diagnostic("error", "IMAGE204", f"Unable to decode WebP image: {path.name}")) from exc
        return target
    ...
    def _convert_svg(self, path: Path) -> Path:
        ...
        try:
            ...
            raise MddocxError(Diagnostic("error", "IMAGE208", "SVG conversion requires the mddocx[images] extra.")) from exc
        try:
            root = DET.fromstring(data)
        except Exception as exc:
            raise MddocxError(Diagnostic("error", "IMAGE206", f"Unsafe or invalid SVG: {path.name}")) from exc
        for element in root.iter():
            for key, value in element.attrib.items():
                local = key.rsplit("}", 1)[-1].lower()
                if local in {"href", "src"} and value and not value.startswith(("data:", "#")):
                    raise MddocxError(Diagnostic("error", "IMAGE207", "External references inside SVG are blocked."))
                for match in re.finditer(r"url\(\s*['\"]?([^)'\"]+)", value or "", flags=re.I):
                    target = match.group(1).strip()
                    if not target.startswith(("#", "data:")):
                        raise MddocxError(Diagnostic("error", "IMAGE207", "External CSS references inside SVG are blocked."))
            text = element.text or ""
            if "@import" in text.lower():
                raise MddocxError(Diagnostic("error", "IMAGE207", "CSS imports inside SVG are blocked."))
        ...
        if target.exists():
            return target
        try:
            cairosvg.svg2png(bytestring=data, write_to=str(target), dpi=self.images.svg_dpi, unsafe=False)
        except Exception as exc:
            raise MddocxError(Diagnostic("error", "IMAGE209", f"Unable to convert SVG: {path.name}")) from exc
        return target
