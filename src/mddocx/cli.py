...
from .config import (
    ...
    TOCConfig,
    ValidationConfig,
    NotesConfig, CitationConfig, ReferenceConfig, HeadingNumberingConfig, TitlePageConfig,
    AbstractConfig, CodeConfig, CommentConfig, MetadataConfig,
)
...
from .project import build_project, compile_project, init_project, load_project, project_info, watch_project, ProjectWatchEvent
from .visual_qa import (
    VisualQAUnavailable, compare_visual_pages, render_docx_pages, update_visual_baseline
)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mddocx", description="Compile Markdown into native editable Microsoft Word DOCX.", epilog="Interactive: mddocx shell | Project: mddocx build/watch/project | QA: mddocx inspect/doctor/accessibility")
    p.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    p.add_argument("input", nargs="+", type=Path)
    p.add_argument("-o", "--output", type=Path)
    p.add_argument("--output-dir", type=Path, help="Batch output directory; enables multi-input conversion")
    p.add_argument("--recursive", action="store_true", help="Recursively include .md files from input directories")
    p.add_argument("--fail-fast", action="store_true", help="Stop batch conversion after the first failure")
    ...
    p.add_argument("--template", type=Path, help="Use an existing DOCX/DOTX as the document template")
    ...
    p.add_argument("--keyword", action="append", default=[], help="Document keyword; may be repeated")
    p.add_argument("--heading-numbering", action="store_true", help="Use native multilevel numbering for headings")
    ...
    p.add_argument("--different-first-page", action="store_true", help="Enable separate first-page header/footer")
    p.add_argument("--different-odd-even", action="store_true", help="Enable separate even-page header/footer")
    ...
    p.add_argument("--notes", choices=["footnote", "endnote"], default="footnote", help="Render Markdown notes as native Word footnotes or endnotes")
    p.add_argument("--bibliography", type=Path, help="BibTeX or CSL-JSON bibliography file")
    p.add_argument("--citation-style", choices=["author-year", "apa", "ieee", "numeric"], default="author-year")
    p.add_argument("--auto-bibliography", action="store_true", help="Append a bibliography when citations are present")
    p.add_argument("--no-crossrefs", action="store_true", help="Disable numbered caption/cross-reference features")
    ...
    p.add_argument("--code-line-numbers", action="store_true", help="Add editable line numbers to code blocks")
    p.add_argument("--code-language-labels", action="store_true", help="Show code-language labels")
    p.add_argument("--no-syntax-highlighting", action="store_true", help="Disable editable Pygments token highlighting")
    p.add_argument("--comment-author", default="mddocx", help="Author for CriticMarkup native Word comments")
    ...
    p.add_argument("--auto-landscape-tables", action="store_true", help="Place wide tables in landscape sections")
    p.add_argument("--no-mermaid", action="store_true", help="Keep Mermaid fences as editable code instead of rendering supported diagrams")
    p.add_argument("--mermaid-strict", action="store_true", help="Fail instead of falling back to code for unsupported Mermaid syntax")
    p.add_argument("--allow-remote-resources", action="store_true", help="Allow safe HTTPS image downloads")
    p.add_argument("--allow-domain", action="append", default=[], help="Restrict remote image hosts; may be repeated")
    p.add_argument("--plugin", action="append", default=[], metavar="NAME", help="Load an explicitly named mddocx.extensions entry point")
    p.add_argument("--ast-cache", nargs="?", const=".mddocx-cache", metavar="DIR", help="Enable persistent normalized-AST cache")
    p.add_argument("--max-input-bytes", type=int, default=20_000_000)
    p.add_argument("--no-reproducible", action="store_true", help="Do not normalize DOCX ZIP metadata/order")
    p.add_argument("--no-validate-output", action="store_true", help="Skip generated DOCX package validation")
    p.add_argument("--diagnostics-json", nargs="?", const="-", metavar="PATH", help="Write diagnostics JSON (default: stdout)")
    p.add_argument("--diagnostics-sarif", nargs="?", const="-", metavar="PATH", help="Write SARIF 2.1.0 diagnostics (default: stdout)")
    p.add_argument("--profile", action="store_true", help="Print render timing, hash, cache and memory statistics as JSON")
    p.add_argument("--profile-memory", action="store_true", help="Track peak Python allocation memory")
    p.add_argument("--ai-metadata", choices=["auto", "strip", "keep"], default="auto", help="AI/chat export metadata handling: auto-strip high-confidence metadata (default), force strip, or keep")
    p.add_argument("--open", action="store_true", help="Open the generated DOCX with the operating-system default application")
    return p
...
def _config_from_args(args: argparse.Namespace) -> RenderConfig:
    return RenderConfig(
        ...
        title=args.title,
        author=args.author,
        title_page=TitlePageConfig(enabled=args.title_page or bool(args.subtitle) or bool(args.organization), subtitle=args.subtitle, organization=args.organization),
        abstract=AbstractConfig(text=args.abstract_text, keywords=tuple(args.keyword)),
        heading_numbering=HeadingNumberingConfig(enabled=args.heading_numbering, max_level=max(1, min(6, args.heading_numbering_depth))),
        toc=TOCConfig(enabled=args.toc),
        header=HeaderConfig(
            enabled=bool(args.header) or bool(args.first_header) or bool(args.even_header), text=args.header,
            different_first_page=args.different_first_page, different_odd_even=args.different_odd_even,
            first_page_text=args.first_header, even_page_text=args.even_header,
        ),
        footer=FooterConfig(
            enabled=bool(args.footer) or args.page_numbers or args.num_pages or args.page_x_of_y or bool(args.first_footer) or bool(args.even_footer),
            text=args.footer, page_number=args.page_numbers, num_pages=args.num_pages, page_x_of_y=args.page_x_of_y,
            different_first_page=args.different_first_page, different_odd_even=args.different_odd_even,
            first_page_text=args.first_footer, even_page_text=args.even_footer,
        ),
        code=CodeConfig(syntax_highlighting=not args.no_syntax_highlighting, line_numbers=args.code_line_numbers, show_language_label=args.code_language_labels),
        ...
        mermaid=MermaidConfig(enabled=not args.no_mermaid, fallback="error" if args.mermaid_strict else "code"),
        ...
        performance=PerformanceConfig(enabled=args.profile or args.profile_memory, track_memory=args.profile_memory),
        ...
        cache=CacheConfig(ast_enabled=bool(args.ast_cache), directory=Path(args.ast_cache) if args.ast_cache else None),
        plugins=PluginConfig(names=tuple(args.plugin)),
        notes=NotesConfig(style=args.notes),
        citations=CitationConfig(bibliography=args.bibliography, style=args.citation_style, auto_bibliography=args.auto_bibliography),
        references=ReferenceConfig(
            enabled=not args.no_crossrefs, captions=not args.no_crossrefs,
            equation_number_format=args.equation_numbering, caption_number_format=args.caption_numbering,
        ),
    )
...
def _main_inspect(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx inspect", description="Inspect DOCX structure for common rendering/package defects.")
    p.add_argument("docx", type=Path)
    p.add_argument("--json", action="store_true", help="Emit machine-readable JSON")
    p.add_argument("--strict", action="store_true", help="Return exit code 2 when structural issues are found")
...
def _main_accessibility(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx accessibility", description="Audit a DOCX for common Word accessibility issues.")
    p.add_argument("docx", type=Path)
    p.add_argument("--json", action="store_true", help="Emit machine-readable JSON")
...
def _main_benchmark(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx benchmark", description="Run the deterministic large-document performance gate.")
...
def _main_api(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx api", description="Show the frozen mddocx v1 public API manifest.")
    p.add_argument("--json", action="store_true")
    args = p.parse_args(argv)
...
def _main_doctor(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx doctor", description="Check the local mddocx runtime and optional QA capabilities.")
    p.add_argument("--json", action="store_true", help="Emit machine-readable JSON")
    args = p.parse_args(argv)
    report = run_doctor()
    print(report.to_json() if args.json else report.to_text())
...
def _main_visual_qa(argv: list[str]) -> int:
    ...
    p = argparse.ArgumentParser(prog="mddocx visual-qa", description="Render a DOCX to pages and compare it with a visual baseline.")
    p.add_argument("docx", type=Path)
    p.add_argument("--baseline-dir", type=Path, required=True)
    p.add_argument("--threshold", type=float, default=0.985)
    p.add_argument("--dpi", type=int, default=144)
    p.add_argument("--update-baseline", action="store_true", help="Replace the baseline with the current rendered pages")
    p.add_argument("--json", action="store_true")
    args = p.parse_args(argv)
    try:
...
def _main_template(argv: list[str]) -> int:
    if not argv or argv[0] != "inspect":
        ...
        return 2
    p = argparse.ArgumentParser(prog="mddocx template inspect", description="Inspect a Word template's page setup, styles, and headers/footers.")
...
def _main_build(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx build", description="Build a multi-file mddocx project from mddocx.yml.")
    p.add_argument("project", nargs="?", default=".", type=Path, help="Project directory or mddocx.yml")
    p.add_argument("-o", "--output", type=Path, help="Override the project output path")
    p.add_argument("--force", action="store_true", help="Rebuild even when all dependencies are unchanged")
    p.add_argument("--check", action="store_true", help="Parse the complete project without writing DOCX")
    ...
    try:
        ...
        add_recent(source=result.project_file, output=result.output_path, action="build", workspace=manifest.root, settings={"fingerprint": result.fingerprint})
        if args.open:
...
def _main_watch(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx watch", description="Watch a project and rebuild its DOCX when inputs change.")
    p.add_argument("project", nargs="?", default=".", type=Path, help="Project directory or mddocx.yml")
    p.add_argument("-o", "--output", type=Path, help="Override the project output path")
    p.add_argument("--interval", type=float, help="Polling interval in seconds")
    p.add_argument("--once", action="store_true", help="Build once using the watch pipeline, then exit")
    ...
    try:
        ...
        def show(event: ProjectWatchEvent) -> None:
            ...
            if event.kind == "change":
                names = ", ".join(str(p.relative_to(manifest.root)) if p.is_relative_to(manifest.root) else str(p) for p in event.changed[:8])
...
def _main_project(argv: list[str]) -> int:
    ...
    command = argv[0]
    if command == "init":
        p = argparse.ArgumentParser(prog="mddocx project init", description="Create a starter mddocx.yml project.")
        ...
        try:
            ...
            return 2
    if command == "info":
        p = argparse.ArgumentParser(prog="mddocx project info", description="Show project sources and resolved dependencies.")
...
def _main_data(argv: list[str]) -> int:
    if not argv or argv[0] != "inspect":
        ...
        return 2
    p = argparse.ArgumentParser(prog="mddocx data inspect", description="Inspect a CSV/JSON data source before using it in a chart or imported table.")
...
def _main_metadata(argv: list[str]) -> int:
    if not argv or argv[0] not in {"inspect", "clean"}:
        print("Usage: mddocx metadata inspect FILE [--policy auto|strip|keep] [--json]\n       mddocx metadata clean FILE -o CLEAN.md [--policy auto|strip|keep]", file=sys.stderr)
        return 2
    command = argv[0]
    p = argparse.ArgumentParser(prog=f"mddocx metadata {command}", description="Inspect or remove AI/chat export metadata from Markdown without executing or rendering it.")
    ...
    if args.json:
        ...
        print(f"Removed front-matter fields: {', '.join(r.removed_front_matter_keys) if r.removed_front_matter_keys else '(none)'}")
        if r.reasons:
            print("Reasons:")
...
def _main_shell(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx shell", description="Open the interactive mddocx console.")
...
def _main_recent(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx recent", description="Show recently generated DOCX files. Paths/settings only are stored; document content is never stored.")
...
def _main_open(argv: list[str]) -> int:
    p = argparse.ArgumentParser(prog="mddocx open", description="Open a DOCX using the operating-system default application.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        batch_mode = len(sources) > 1 or any(p.is_dir() for p in args.input) or args.output_dir is not None
        if args.check:
            status = 0
        ...
        if batch_mode:
            if args.output is not None:
                print("ERROR: --output cannot be combined with batch conversion; use --output-dir", file=sys.stderr)
                return 2
        ...
        print(output)
        add_recent(source=source, output=output, action="render", workspace=Path.cwd(), settings={"theme": args.theme})
        if args.open:
            open_path(output)
