...
def add_field(
    paragraph, instruction: str, placeholder: str | None = None, dirty: bool = True
) -> None:
...
def add_toc(paragraph, min_level: int = 1, max_level: int = 3) -> None:
    ...
    add_field(
        paragraph, f' TOC \\o "{minimum}-{maximum}" \\h \\z \\u ', "Update this field in Word"
    )
...
def add_section_equation_number(
    paragraph, section_placeholder: str, equation_placeholder: str, bookmark: str, bookmark_id: int
) -> None:
    start = OxmlElement("w:bookmarkStart")
    start.set(qn("w:id"), str(bookmark_id))
    start.set(qn("w:name"), bookmark)
...
_FIELD_TOKEN_RE = re.compile(
    r"\{(PAGE|NUMPAGES|DATE|CREATEDATE|SAVEDATE|AUTHOR|SECTION|SECTIONPAGES|FILENAME|TITLE|SUBJECT|DOCPROPERTY:[A-Za-z0-9 _.-]+)\}",
    re.I,
)
