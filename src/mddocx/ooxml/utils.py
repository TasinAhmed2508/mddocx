...
def set_repeat_table_header(row) -> None:
    ...
    if existing is None:
        existing = OxmlElement("w:tblHeader"); tr_pr.append(existing)
...
def set_paragraph_bottom_border(paragraph, color: str = "B7B7B7", size: str = "6") -> None:
    ...
    bottom.set(qn("w:val"), "single"); bottom.set(qn("w:sz"), size); bottom.set(qn("w:space"), "1"); bottom.set(qn("w:color"), color)
    p_bdr.append(bottom)
