...
def _build_footnotes_xml(notes: dict[int, list], math_converter) -> bytes:
    ...
    for footnote_id, nodes in sorted(notes.items()):
...
def _separator(footnote_id: int, kind: str, element_name: str):
    fn = OxmlElement("w:footnote")
...
def _patch_content_types(data: bytes) -> bytes:
    ...
    if not any(el.get("PartName") == "/word/footnotes.xml" for el in root):
